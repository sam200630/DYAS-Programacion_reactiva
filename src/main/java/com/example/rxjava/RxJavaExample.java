...
public class RxJavaExample {
    public static void main(String[] args) {
        ...
        observable.filter(item -> item % 2 == 0) // Filtrar los números pares
                ...
                .flatMap(item -> Observable.just(item).subscribeOn(Schedulers.computation())) // Emitir en un hilo de
                                                                                              // computación
                .doOnNext(item -> System.out.println("Processing item on thread: " + item)) // Loggear el hilo de
                                                                                            // ejecución
